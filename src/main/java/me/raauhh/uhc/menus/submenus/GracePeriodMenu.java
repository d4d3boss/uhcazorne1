...
public class GracePeriodMenu extends ChestMenu<UHC> {

    private GameManager gameManager;

    public GracePeriodMenu() {
        super("Grace Period ┃ ", 9);
    ...
    public void update(Player player) {
        ...
        inventory.setItem(3, new ItemUtil(Material.DIAMOND_SWORD).setName("§eGrace Period §7(§f" + Common.calculate(gameManager.getPvpTime()) + "§7)").get());
    ...
    private String getTitle() {
        return "Grace Period ┃ §7" + Common.calculate(gameManager.getPvpTime());
    }
        ...
        if (topInventory.equals(clickedInventory)) {
            ...
            switch (material) {
                case INK_SACK:
                    switch (itemName) {
                        case "+1":
                            gameManager.setPvpTime(gameManager.getPvpTime() + 60);
                            break;
                        case "+5":
                            gameManager.setPvpTime(gameManager.getPvpTime() + (5 * 60));
                            break;
                        case "-1":
                            if (((gameManager.getPvpTime() / 60) - 1) < 0) return;
                            gameManager.setPvpTime(gameManager.getPvpTime() - 60);
                            break;
                        case "-5":
                            if (((gameManager.getPvpTime() / 60) - 5) < 0) return;
                            gameManager.setPvpTime(gameManager.getPvpTime() - (5 * 60));
                            break;
                    }
                    update(player);
