...
public class BorderTimeMenu extends ChestMenu<UHC> {

    private GameManager gameManager;

    public BorderTimeMenu() {
        super("Border Time ┃ ", 9);
    ...
    public void update(Player player) {
        ...
        inventory.setItem(3, new ItemUtil(Material.BEDROCK).setName("§eBorder Time §7(§f" + Common.calculate(gameManager.getBorderTime()) + "§7)").get());
    ...
    private String getTitle() {
        return "Border Time ┃ §7" +  Common.calculate(gameManager.getBorderTime());
    }
        ...
        if (topInventory.equals(clickedInventory)) {
            ...
            switch (material) {
                case INK_SACK:
                    switch (itemName) {
                        case "+1":
                            gameManager.setBorderTime(gameManager.getBorderTime() + 60);
                            break;
                        case "+5":
                            gameManager.setBorderTime(gameManager.getBorderTime() + (5 * 60));
                            break;
                        case "-1":
                            if (((gameManager.getBorderTime() / 60) - 1) < 0) return;
                            gameManager.setBorderTime(gameManager.getBorderTime() - 60);
                            break;
                        case "-5":
                            if (((gameManager.getBorderTime() / 60) - 5) < 0) return;
                            gameManager.setBorderTime(gameManager.getBorderTime() - (5 * 60));
                            break;
                    }
                    update(player);
