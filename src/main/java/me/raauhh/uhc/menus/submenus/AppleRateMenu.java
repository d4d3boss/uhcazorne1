...
public class AppleRateMenu extends ChestMenu<UHC> {

    private GameManager gameManager;

    public AppleRateMenu() {
        super("Apple Rate ┃ ", 9);
    ...
    public void update(Player player) {
        ...
        inventory.setItem(3, new ItemUtil(Material.APPLE).setName("§eApple Rate §7(§f" + gameManager.getAppleRatePercent() + "%§7)").get());
    ...
    private String getTitle() {
        return "Apple Rate ┃ §7" + gameManager.getAppleRatePercent() + " %";
    }
        ...
        if (topInventory.equals(clickedInventory)) {
            ...
            switch (material) {
                case INK_SACK:
                    switch (itemName) {
                        case "+1":
                            gameManager.setAppleRatePercent(gameManager.getAppleRatePercent() + 1);
                            break;
                        case "+5":
                            gameManager.setAppleRatePercent(gameManager.getAppleRatePercent() + 5);
                            break;
                        case "-1":
                            if ((gameManager.getAppleRatePercent() - 1) < 1 || (gameManager.getAppleRatePercent() + 1) > 100)
                                return;
                            gameManager.setAppleRatePercent(gameManager.getAppleRatePercent() - 1);
                            break;
                        case "-5":
                            if ((gameManager.getAppleRatePercent() - 5) < 1 || (gameManager.getAppleRatePercent() + 5) > 100)
                                return;
                            gameManager.setAppleRatePercent(gameManager.getAppleRatePercent() - 5);
                            break;
                    }
