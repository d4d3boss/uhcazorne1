...
public class VisualiseHandler {
    ...
    private static List<Material> passthrough = new ArrayList<Material>();
        ...
        synchronized (storedVisualises) {
            ...
            if (filled != null) {
                ...
                for (Location location : locations) {
                    ...
                    if (player.getWorld() == UHC.getInstance().getWorldManager().getUhcWorldNether()) passthrough.add(Material.BEDROCK);
                    if (!canOverwrite && storedVisualises.contains(player.getUniqueId(), location)
                            || (previousType = location.getBlock().getType()).isSolid()
                            || !passthrough.contains(previousType)) continue;

                    passthrough.remove(Material.BEDROCK);
